...
class Student:
    ...
    def update(self, student_id, name, marks):
        self.database.execute("UPDATE students SET name = %s, marks = %s WHERE id = %s", (name, marks, student_id))
        self.database.commit()
    ...
    def retrieve(self):
        return self.database.fetch_all("SELECT id, name, age, course, marks FROM students ORDER BY id")
